...
angle_ = np.linspace(0, 2*pi, 181)[:180]
for i, angle in enumerate(angle_):
    C = np.array([cos(angle), sqrt(3)/2], dtype=float)
    t = Triangle(A, B, C)
    Mcircles = list(t.malfatti_circles()["circles"].values())
    triangle = Triangle(Mcircles[0].center, Mcircles[1].center, Mcircles[2].center)
    soddyO = soddyOuterCircle(triangle)
...
os.system(
    "magick convert -dispose previous -loop 0 -delay 7 zpic_*.png MalfattiApollonian.gif"    
) 